...
class ChatDashscope(ChatOpenAI):
    """具有推理能力的扩展后的ChatOpenAI模型。

    这个类扩展了基本的ChatOpenAI模型，以支持OpenAI的推理模型，该模型在其响应中包含reasoning_content。
    It handles the extraction and preservation of reasoning content during both streaming and non-streaming operations.
    它在streaming和non-streaming 期间处理推理内容的提取和保存。
    """
